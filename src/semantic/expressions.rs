...
impl<'a> ExpressionVisitor<'a> for SemanticVisitor {
    type ExprReturnType = Option<Type>;
    ...
    fn visit_expr(&mut self, expr: &mut ExprKind) -> Self::ExprReturnType {
        match expr {
            ...
            ExprKind::StructInit(name, fields) => {
                let struct_ty = self.structs.get(name.data(&self.source)).unwrap().to_owned();
                for (field, expr) in fields.iter_mut().enumerate() {
                    let ty1 = self.visit_expr(&mut expr.kind).unwrap();
                    let ty2 = &struct_ty.get(field).unwrap().1;
                    self.type_check(&ty1, ty2);
                }
                Some(TypeKind::Struct(name.data(&self.source).to_string()).into())
            }
            ExprKind::FieldAccess(struct_, field) => {
                let struct_ty = self.visit_expr(&mut struct_.kind).unwrap();
                let data: Rc<str> = field.data(&self.source).into();
                if let TypeKind::Struct(name) = *struct_ty.kind {
                    let struct_ty = self.structs.get(name.as_str()).unwrap().to_owned();
                    let field_ty = &struct_ty.iter().find(|(f, _)| f == &data).unwrap().1;
            ...
            ExprKind::Call(func, params) => {
                let res = self.fn_decls.get(func.data(&self.source));
                if res.is_none() {
            ...
            ExprKind::Var(v) => Some(self.values.get(&v.data(&self.source).into()).unwrap().to_owned()),
            ExprKind::Break => {
    ...
    fn visit_binary(
        &mut self,
        _expr: &mut crate::parser::ast::expressions::ExprKind,
    ) -> Self::ExprReturnType {
        todo!()
    }
